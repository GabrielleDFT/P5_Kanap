...
function deleteItem() {
    ...
    for (let j = 0; j < deleteBttn.length; j++){
        deleteBttn[j].addEventListener("click" , (event) => {
            event.preventDefault();
            // Select & Delete Item by Id & Color
...
function validForm() {
    ...
    //-----Create Function/Pop-ups for Validation : First Name, Last Name, Address, City & Email
    ...
    const validAddress = function(inputAddress) {
        ...
        if (addressRegex.test(inputAddress.value)) {
            ...
            addressErrorMsg.innerHTML = 'Veuillez renseigner une adresse postale correcte'; }
    };
    const validCity = function(cityPlace) {
        ...
        if (regex.test(cityPlace.value)) {
            ...
            cityErrorMsg.innerHTML = 'Veuillez renseigner correctement le nom de votre ville'; }
    };
